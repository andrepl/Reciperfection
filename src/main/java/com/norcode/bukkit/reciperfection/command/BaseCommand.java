...
public class BaseCommand implements TabExecutor {
    protected Reciperfection plugin;
    private String name;
    ...
    public BaseCommand(Reciperfection plugin, String name, String[] aliases, String requiredPermission, String[] help) {
        this.plugin = plugin;
        this.name = name;
    ...
    protected void showHelp(CommandSender sender, String label, LinkedList<String> args) {
        if (help != null && help.length > 0) {
			...
			for (String sc: subs.keySet()) {
				BaseCommand scmd = subs.get(sc);
				sender.sendMessage(ChatColor.AQUA + sc + ChatColor.GRAY + "" +
						((scmd.help == null || scmd.help.length == 0) ? "" : " - " + scmd.help[0]));
			}
		}
    }
